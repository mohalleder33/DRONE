...
import { getOfficers, updateOfficer, deleteOfficer } from '../../services/personnelService';
...
const OfficersList = ({ user, onRefresh }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getOfficers(pagination.page, pagination.limit, { search, attendanceStatus: statusFilter });
      setData(res.data.data);
      setPagination(res.data.pagination);
    } catch (error) { 
      toast.error('فشل تحميل الضباط'); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleStatusChange = async (item, newStatus) => {
    ...
    try {
      await updateOfficer(id, { attendanceStatus: newStatus });
  ...
  const handleDelete = async (item) => {
    const id = item._id || item.id;
    if (window.confirm('هل أنت متأكد من حذف هذا الضابط؟')) {
      try {
        ...
        <div className="flex gap-2">
          {user?.role === 'admin' && (
            <button onClick={() => setShowAddModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md flex items-center gap-2 transition">
              <PlusIcon className="h-5 w-5" /> إضافة ضابط
            </button>
          )}
