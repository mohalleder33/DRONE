...
import { createCourse, getAvailablePersonnel } from '../../services/trainingCourseService';
import { ChevronDownIcon, ChevronUpIcon, UserPlusIcon, UserGroupIcon } from '@heroicons/react/24/outline';
...
const AddCourseModal = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchPersonnel = async () => {
      try {
        ...
        const all = res.data;
        setAvailableOfficers(all.filter(p => p.type === 'officers'));
        setAvailableNCOs(all.filter(p => p.type === 'ncos'));
        setAvailableRecruits(all.filter(p => p.type === 'recruits'));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createCourse({ ...form, trainees: selectedTrainees });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto z-50">
      <div className="flex items-center justify-center min-h-screen p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
          <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">➕ إضافة دورة جديدة</h2>
          <form onSubmit={handleSubmit}>
            {/* الحقول الأساسية */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <input name="courseName" placeholder="اسم الدورة *" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" required />
              <input name="courseNumber" placeholder="رقم الدورة *" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" required />
              <input name="startDate" type="date" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" required />
              <input name="endDate" type="date" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" required />
              <input name="adminSupervisor" placeholder="المشرف الإداري" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" />
              <input name="militarySupervisor" placeholder="المشرف العسكري" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" />
              <input name="location" placeholder="الموقع" defaultValue="الرئاسة" onChange={handleChange} className="border p-2 rounded dark:bg-gray-700" />
            </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded">إنشاء</button>
            </div>
          </form>
        </div>
      </div>
